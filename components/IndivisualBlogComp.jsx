...
export default function IndivisualPlastics({
  ...
  const parsedContent = parseContent(detailedBlog?.attributes?.content);
  ...
  return (
    <article>
      ...
      <Image
        src={
          detailedBlog &&
            detailedBlog?.attributes?.thumbnail?.image?.data?.attributes?.url
            ? detailedBlog &&
            detailedBlog?.attributes?.thumbnail?.image?.data?.attributes?.url
            : ""
        }
            ...
            {parsedContent.map((item, index) => {
              if (item.type === "h1") {
                ...
                if (imageItems.length >= 3) {
                  ...
                  return (
                    <div key={index}>No sufficient images to display.</div>
                  );
                }
            ...
            {relatedUpNextData &&
              relatedUpNextData?.sort(
                ...
                <>
                  ...
                  >
                    <Link
                      href={`/blogs/${nextData?.attributes?.slug}`}
                      className="w-full md:pr-6 lg:pr-0 pr-0"
                    >
                      <Image
                        src={nextData?.attributes?.thumbnail?.image?.data?.attributes?.url}
                        height={350}
                        width={350}
                        alt=""
